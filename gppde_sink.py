...
beta_ori = np.zeros(10)
beta_wei = np.zeros(10)
for q in range(10):
    dz = -0.025
    ...
    skip = 0.15
    for i in range(ny):
        if (abs(X[i, 0]) <= skip) or (abs(X[i, 0]) >= 3-0.025-skip) or (X[i, 1] <= 86400 * 4):
    ...
    def sse(beta,lm,  resid, X_z, X_t, h, sink_para, w):
        residfull = resid + sink(X_z, X_t, h, sink_para, beta, lm)
        w = w/ np.sum(w)
        return(np.sum(w * residfull ** 2))
    w = np.full(len(resid), 1.0)
    opt_ini = minimize(fun=sse, x0 =np.array([2.0]) ,args= (lm, resid, X_z, X_t, h, sink_para, w),method = 'Nelder-Mead') 
    beta_gp = opt_ini.x
    beta_ori[q] = beta_gp[0]
    
    def beta_weight(z, t, h, sink_para, beta, lm):
        nz = len(z)
        tp = sink_para[:, 0]
        zr = sink_para[:, 1]
        a1 = sink_para[:, 2]
        ...
        a3 = sink_para[:, 4]
        a4 = sink_para[:, 5]
        beta_weight = np.zeros(nz)
        for i in range(nz):
            index = int(t[i]/86400 - 1)
            if (abs(z[i]) <= lm[0] * zr[index]):
                alph = alpha_func(h[i] , a1[index], a2[index], a3[index], a4[index])
                const = alph * tp[index] / (lm[0] *zr[index]) 
                expo = pow(1.0 - abs(z[i]) / (lm[0] * zr[index]), beta[0])
                deri = const * (expo + (1.0 + beta[0]) * np.log(1.0 - abs(z[i]) / (lm[0] * zr[index])) * expo)
                if (deri != 0 ):
                    beta_weight[i] =  1.0 / abs(deri) 
        if (np.sum(beta_weight) != 0):
            return beta_weight
        else:
            return np.full(nz, 1.0)
    
    def lm_weight(z, t, h, sink_para, beta, lm):
        nz = len(z)
        tp = sink_para[:, 0]
        zr = sink_para[:, 1]
        a1 = sink_para[:, 2]
        ...
        a3 = sink_para[:, 4]
        a4 = sink_para[:, 5]
        lm_weight = np.zeros(nz)
        for i in range(nz):
            index = int(t[i]/86400 - 1)
            if (abs(z[i]) <= lm[0] * zr[index]):
                alph = alpha_func(h[i] , a1[index], a2[index], a3[index], a4[index])
                const = alph * tp[index] * (1.0 + beta[0]) / zr[index]
                expo = pow(1.0 - abs(z[i]) / (lm[0] * zr[index]), beta[0] - 1.0)
                deri = const * (  1.0 / (lm[0] ** 2) * expo * ((beta[0] + 1) * abs(z[i]) / (lm[0] * zr[index]) - 1) )
                if (deri != 0 ):
                    lm_weight[i] =  1.0 / abs(deri) 
        if (np.sum(lm_weight) != 0):
            return lm_weight
        else:
            return np.full(nz, 1.0)
        
    w = beta_weight(X_z, X_t, h, sink_para, beta_gp, lm)
    
    opt_las = minimize(fun=sse, x0 =beta_gp ,args= (lm, resid, X_z, X_t, h, sink_para, w),method = 'Nelder-Mead') 
    beta_wei[q] = opt_las.x[0]
print("ori beta", beta_ori)
print("weighted beta", beta_wei)
   


# def ssre(est_para, alpha, m, theta, dthetadt, dthetadz, d2thetadz2, S, k_s, theta_r, theta_s, X, sink_para) :
#     beta, lm = est_para
#     h = np.vectorize(h_)(theta, theta_r, theta_s, 1/(1-m), m, alpha)
#     K = np.vectorize(k_)(h, S, k_s, m)
#     z = X[:, 0]
#     t = X[:, 1]
#     resid = dthetadt - (term1(dthetadz, S, k_s, h, alpha, m, theta_r, theta_s) + K * term2(dthetadz, d2thetadz2, S, h, alpha, m, theta_r, theta_s) + term3(dthetadz, S, k_s, m, theta_r, theta_s)) + sink(z, t, h, sink_para, beta, lm)
#     return(np.sum(resid**2))

# s_obs = np.vectorize(S_)(post_mean, theta_r, theta_s)
# other_param = (alpha, m, post_mean, dydt, dydz, d2ydz2, s_obs, k_s, theta_r, theta_s, X, sink_para)
# print("initial SSR =", ssre(np.array([2.0, 1.35]), alpha, m, post_mean, dydt, dydz, d2ydz2, s_obs, k_s, theta_r, theta_s, X, sink_para))
...
# opt_res = minimize(fun=ssre, x0=np.array([1.5, 1.35]), bounds = par_range, args=other_param, method='Nelder-Mead')
# beta_gp, lm_gp = opt_res.x
# print("param =", beta_gp, lm_gp)
# print("minSSR =", opt_res.fun)

# h_post = np.vectorize(h_)(post_mean, theta_r, theta_s, 1/(1-m), m, alpha)
# z_sub = X[:, 0]
# t_sub = X[:, 1]
# tru_sink = sink(z_sub, t_sub, h_post, sink_para, 2.0, 1.35)
# est_sink = sink(z_sub, t_sub, h_post, sink_para, beta_gp, lm_gp)
# print('sink_diff=', np.max(np.abs(tru_sink - est_sink)))
